...
- 고용보험 처리
- 월지급 계산 (v2.0 UI)
- 위하고 업로드 자동화 (Excel 파일 자동 생성)
- 월지급DB 저장 (원본 데이터 자동 삭제)
- 대량이체 등록
- 통합 급여 처리
...
### 개요
완전 자동화된 급여 처리 시스템으로 월지급 계산부터 위하고 업로드, 대량이체까지 원스톱 처리가 가능합니다.
...
2️⃣ 급여 관리 > 월지급 계산
    ...
    ├─ 근무위치 필터링
    ├─ 월지급계산 시트 생성
    └─ 완료 시 위하고 업로드 안내 메시지 표시
    ↓
3️⃣ 급여 관리 > 위하고 업로드 > 파일 업데이트
    ├─ 월지급더존업로드 데이터 읽기
    ├─ Google Sheets 템플릿 업데이트
    ├─ Excel 파일 자동 생성
    └─ 완료 메시지 및 파일 위치 안내
    ↓
4️⃣ 위하고 급여입력 (외부 시스템)
...
6️⃣ 급여 관리 > 월지급DB 저장
    ...
    ├─ 신규 직원만 선택하여 저장
    ├─ 월지급DB 시트에 저장
    └─ 원본(월급여더존다운로드) 데이터 자동 삭제
    ↓
7️⃣ 급여 관리 > 대량이체 등록
    ├─ UI에서 급여기준월 선택
    ├─ 은행정보 검증
    ├─ 대량이체 시트 생성
    └─ 월지급DB 입금처리여부 자동 업데이트
```

### 주요 기능

#### 1. 월지급 계산 (v2.0)
**파일**: `src/payroll/monthlyPayrollUIHandler.js`, `html/monthlyPayrollUI.html`

**기능**:
- 급여기준월 입력 (날짜 선택)
- 직원 목록 표시 (이름, 지급액, 공제액, 차인지급액)
- 검색 기능 (직원 이름 검색)
- 근무위치 필터링 (전체, 제주 본점, 창원, 김해)
- 전체선택/전체해제
- 통계 표시 (선택 인원 수, 총 지급액, 총 공제액)
- 월지급계산 시트 자동 생성
- 완료 시 위하고 업로드 안내 메시지
...
#### 2. 위하고 업로드 자동화
**파일**: `src/payroll/updateWihagoUpload.js`

**기능**:
- 월지급더존업로드 데이터 자동 읽기 (2행부터)
- Google Sheets 템플릿 자동 업데이트
- 3행 이후 기존 데이터 자동 삭제
- 새 데이터 붙여넣기 (값만)
- Excel 파일 (.xlsx) 자동 생성
- 기존 Excel 파일 자동 교체
...
**사용 방법**:
```
급여 관리 > 5. 위하고 업로드 > 파일 업데이트
→ 확인 메시지 "예" 클릭
→ Google Sheets 업데이트 및 Excel 생성
→ 완료 메시지에서 파일 위치 확인

Excel 파일을 찾을 수 없는 경우:
급여 관리 > 5. 위하고 업로드 > Excel 파일 찾기
→ 파일 URL 복사하여 브라우저에서 열기
```

#### 3. 월지급DB 저장 (자동 정리)
**파일**: `src/payroll/savePayrollDB.js`, `html/savePayrollDBUI.html`
...
#### 4. 대량이체 등록 (v3.0 호환)
**파일**: `src/payroll/bulkTransferHandler.js`, `html/bulkTransferUI.html`

**기능**:
- 급여기준월별 데이터 조회
- 은행정보 검증 (은행명, 계좌번호)
- 대량이체 시트 자동 생성
- 월지급DB 입금처리여부 자동 업데이트 (AC열 → "처리")
- v3.0 스키마 호환 (32컬럼 구조)
...
#### 월지급계산 시트
- 급여기준월별 계산 결과 저장
- 컬럼: 사원코드, 사원명, 지급항목들, 공제항목들, 차인지급액, 은행정보 등

#### 월지급더존업로드 시트
- 위하고 업로드용 임시 데이터
- 월지급계산에서 수동 복사
- 위하고 업로드 자동화로 Excel 변환

#### 월급여더존다운로드 시트
- 위하고에서 다운로드한 데이터
- 월지급DB 저장 후 자동 삭제 (2행 이후)

#### 월지급DB 시트 (v3.0)
- 전체 급여 마스터 데이터
- 32컬럼 구조 (사원코드, 사원명, 지급/공제 항목, 은행정보, 입금처리여부 등)
- 급여기준월별 누적 저장

#### 대량이체 시트
- 은행 대량이체용 포맷
- 컬럼: 입금은행, 계좌번호, 금액, 받는사람
...
#### 중복 데이터가 표시될 때
- 월지급DB에 이미 해당 월의 데이터가 있는 경우
- ⚠ 중복 표시된 직원은 선택 불가
- 신규 직원만 선택하여 저장

#### 대량이체 데이터가 없을 때
- 월지급DB에 해당 월 데이터가 있는지 확인
- 은행정보(AD, AE열)가 입력되어 있는지 확인
- v3.0 스키마로 업데이트되었는지 확인

### 향후 개선 계획
- [ ] 월지급계산 자동화 (더존 다운로드 → 월지급계산)
- [ ] 위하고 API 연동 (수동 업로드 제거)
- [ ] 대량이체 파일 자동 생성 (은행별 포맷)
- [ ] 급여명세서 자동 생성 및 이메일 발송
- [ ] 급여 이력 조회 및 통계 대시보드
