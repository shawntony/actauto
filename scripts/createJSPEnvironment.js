/**
 * 제이에스파트너스 환경 완전 자동 생성 스크립트
 *
 * 사용법:
 * node scripts/createJSPEnvironment.js
 */
...
async function main() {
  ...
  try {
    ...
    const scriptExists = fs.existsSync(path.join(PROJECT_ROOT, 'src', '제이에스파트너스환경생성.js'));
    ...
    console.log('3. 파일: "제이에스파트너스환경생성.js" 선택\n');
    console.log('4. 함수: "create제이에스파트너스환경" 실행\n');
    console.log('5. 로그에서 다음 정보 확인:');
    console.log('   - 스프레드시트 ID');
    console.log('   - 은행거래내역 폴더 ID');
    console.log('   - 스프레드시트 URL\n');
    console.log('6. 생성된 "법인재무관리_제이에스파트너스" 스프레드시트 열기\n');
    ...
    // Step 6: clasp-jsp.json 생성
    console.log('📝 Step 6: clasp-jsp.json 생성');
...
Co-Authored-By: Claude <noreply@anthropic.com>`;
    ...
    // Step 8: 코드 배포
    console.log('📝 Step 8: 제이에스파트너스 환경에 코드 배포');
