...
function createHSK개발완전자동() {
  ...
  try {
    ...
    let 법인관리 = getFolderByName_HSKDEVELOP(myDrive, '법인관리') || myDrive.createFolder('법인관리');
    let 재무관리 = getFolderByName_HSKDEVELOP(법인관리, '재무관리') || 법인관리.createFolder('재무관리');
    let 법인폴더 = getFolderByName_HSKDEVELOP(재무관리, 법인이름) || 재무관리.createFolder(법인이름);
    let 은행거래내역 = getFolderByName_HSKDEVELOP(법인폴더, '은행거래내역') || 법인폴더.createFolder('은행거래내역');
    ...
    let spreadsheet = getSpreadsheetByName_HSKDEVELOP(법인폴더, spreadsheetName);
...
// 헬퍼 함수들 (HSKDEVELOP 전용)
function getFolderByName_HSKDEVELOP(parentFolder, folderName) {
  const folders = parentFolder.getFoldersByName(folderName);
  return folders.hasNext() ? folders.next() : null;
}

function getSpreadsheetByName_HSKDEVELOP(folder, name) {
