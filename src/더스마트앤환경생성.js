/**
 * 더스마트앤 환경 완전 자동 생성 스크립트
 *
 * 실행 방법:
 * 1. Apps Script 편집기에서 이 파일 열기
 * 2. create더스마트앤완전자동() 함수 실행
 */

function create더스마트앤완전자동() {
  const 법인이름 = '더스마트앤';
  ...
  try {
    ...
    let 법인관리 = getFolderByName_THESMARTN(myDrive, '법인관리') || myDrive.createFolder('법인관리');
    let 재무관리 = getFolderByName_THESMARTN(법인관리, '재무관리') || 법인관리.createFolder('재무관리');
    let 법인폴더 = getFolderByName_THESMARTN(재무관리, 법인이름) || 재무관리.createFolder(법인이름);
    let 은행거래내역 = getFolderByName_THESMARTN(법인폴더, '은행거래내역') || 법인폴더.createFolder('은행거래내역');
    ...
    let spreadsheet = getSpreadsheetByName_THESMARTN(법인폴더, spreadsheetName);
...
// 헬퍼 함수들 (THESMARTN 전용)
function getFolderByName_THESMARTN(parentFolder, folderName) {
  const folders = parentFolder.getFoldersByName(folderName);
  return folders.hasNext() ? folders.next() : null;
}

function getSpreadsheetByName_THESMARTN(folder, name) {
