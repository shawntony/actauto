/**
 * 해림씨앤피 환경 완전 자동 생성 스크립트
 *
 * 실행 방법:
 * 1. Apps Script 편집기에서 이 파일 열기
 * 2. create해림씨앤피완전자동() 함수 실행
 */

function create해림씨앤피완전자동() {
  const 법인이름 = '해림씨앤피';
  ...
  try {
    ...
    let 법인관리 = getFolderByName_HAERIMCNP(myDrive, '법인관리') || myDrive.createFolder('법인관리');
    let 재무관리 = getFolderByName_HAERIMCNP(법인관리, '재무관리') || 법인관리.createFolder('재무관리');
    let 법인폴더 = getFolderByName_HAERIMCNP(재무관리, 법인이름) || 재무관리.createFolder(법인이름);
    let 은행거래내역 = getFolderByName_HAERIMCNP(법인폴더, '은행거래내역') || 법인폴더.createFolder('은행거래내역');
    ...
    let spreadsheet = getSpreadsheetByName_HAERIMCNP(법인폴더, spreadsheetName);
...
// 헬퍼 함수들 (HAERIMCNP 전용)
function getFolderByName_HAERIMCNP(parentFolder, folderName) {
  const folders = parentFolder.getFoldersByName(folderName);
  return folders.hasNext() ? folders.next() : null;
}

function getSpreadsheetByName_HAERIMCNP(folder, name) {
