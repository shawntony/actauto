/**
 * 더스마트앤협동조합 환경 완전 자동 생성 스크립트
 *
 * 실행 방법:
 * 1. Apps Script 편집기에서 이 파일 열기
 * 2. create더스마트앤협동조합완전자동() 함수 실행
 */

function create더스마트앤협동조합완전자동() {
  const 법인이름 = '더스마트앤협동조합';
  ...
  try {
    ...
    let 법인관리 = getFolderByName_THESMARTNMUTUAL(myDrive, '법인관리') || myDrive.createFolder('법인관리');
    let 재무관리 = getFolderByName_THESMARTNMUTUAL(법인관리, '재무관리') || 법인관리.createFolder('재무관리');
    let 법인폴더 = getFolderByName_THESMARTNMUTUAL(재무관리, 법인이름) || 재무관리.createFolder(법인이름);
    let 은행거래내역 = getFolderByName_THESMARTNMUTUAL(법인폴더, '은행거래내역') || 법인폴더.createFolder('은행거래내역');
    ...
    let spreadsheet = getSpreadsheetByName_THESMARTNMUTUAL(법인폴더, spreadsheetName);
    ...
    Logger.log(`ENV_KEY:thesmartnmutual`);
...
// 헬퍼 함수들 (THESMARTNMUTUAL 전용)
function getFolderByName_THESMARTNMUTUAL(parentFolder, folderName) {
  const folders = parentFolder.getFoldersByName(folderName);
  return folders.hasNext() ? folders.next() : null;
}

function getSpreadsheetByName_THESMARTNMUTUAL(folder, name) {
